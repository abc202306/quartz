...
(function (Constants) {
    // Regex patterns
    Constants.REGEX_WIKILINK_WITH_PIPE = /^\[\[(?<fn>[^\|]*?)\|.*?\]\]$/;
    Constants.REGEX_WIKILINK_WITHOUT_PIPE = /^\[\[(?<fn>[^\|]*?)\]\]$/;
    Constants.REGEX_FRONTMATTER_BLOCK = /^---\r?\n[^]*?(?<=\n)---\r?\n/;
    Constants.REGEX_COVER_WIKILINK = /^\[\[(.*?\|)?(?<basename>.*)\.(?<extension>.*)\]\]$/;
    Constants.REGEX_FRONTMATTER_SECTION = (keyword) => new RegExp(`(?<=\n)## ${escapeRegExp(keyword)}\n[^]*`);
    Constants.REGEX_FRONTMATTER_SECTION_REPLACE = (keyword) => new RegExp(`(?<=\n)## ${escapeRegExp(keyword)}\n[^]*?(?=\n##\s)`);
    Constants.REGEX_PROPERTY_EXTRACTOR = /^(gallery-doc-)?((ex|n)hentai-)?(tg-)?/;
    Constants.REGEX_FOLDER_STAT_TABLE = /(?<=\n)## folder-struct\n[^#]*(?=\n##\s)/;
    // Path depth constants
    Constants.MARKDOWN_FILE_PATH_DEPTH = 3;
    // Date format parts
    Constants.DATE_YEAR_END_INDEX = 4;
    Constants.DATE_MONTH_END_INDEX = 7;
    Constants.DATE_DAY_END_INDEX = 10;
    // Logging prefixes
    Constants.LOG_PREFIX_TIMER = 'timer-';
    Constants.LOG_PREFIX_STARTED = 'started:';
    Constants.LOG_PREFIX_ENDED = 'ended:';
    Constants.LOG_STARTED_SCRIPT = '==start';
    Constants.LOG_ENDED_SCRIPT = '==end';
})(Constants || (Constants = {}));
/**
 ...
 */
class DateUtil {
    static instance = SingletonFactory.getInstance('DateUtil', () => new DateUtil());
    static getInstance() {
 ...
 */
class ArrayUtil {
    static instance = SingletonFactory.getInstance('ArrayUtil', () => new ArrayUtil());
    static getInstance() {
        return this.instance;
    }
    /**
 ...
 */
class PathUtil {
    static instance = SingletonFactory.getInstance('PathUtil', () => new PathUtil());
    static getInstance() {
        return this.instance;
    }
    /**
     ...
     */
    comparePathByUploadedDate(path1, path2) {
     ...
     */
    getGalleryItemRepresentationStr(path) {
        const f2 = app.vault.getAbstractFileByPath(path);
        const linktext2 = app.metadataCache.fileToLinktext(f2);
        const fc2 = metadataCacheUtil.getFileCache(f2) || {};
        const postDescription = '';
        const display2 = fc2.frontmatter?.japanese ||
            fc2.frontmatter?.english ||
            linktext2;
        const link2 = display2 === linktext2
            ? `| [[${linktext2}|${linktext2}]]`
     ...
     */
    getNonGalleryNotesStr(nonGalleryNotePaths) {
     ...
     */
    getGalleryItemsSimpleList(galleryNotePaths) {
        const gls = [...galleryNotePaths].sort(this.comparePathByUploadedDate.bind(this));
     ...
     */
    getGalleryItemsGroupedList(galleryNotePaths) {
        const gls = [...galleryNotePaths].sort(this.comparePathByUploadedDate.bind(this));
     ...
     */
    buildYearSection(yearKey, yearGroup) {
     ...
     */
    buildMonthSection(monthKey, monthGroup) {
     ...
     */
    getGalleryItemsStr(galleryNotePaths) {
 ...
 */
class StringUtil {
    static instance = SingletonFactory.getInstance('StringUtil', () => new StringUtil());
    static getInstance() {
        return this.instance;
    }
    /**
    ...
    toFileName(wikilinkStr) {
        return (Constants.REGEX_WIKILINK_WITH_PIPE.exec(wikilinkStr)?.groups?.fn ||
    ...
    getTagCount(tagNameSpaceStr) {
        ...
        const galleryMDFileCaches = app.vault
            .getMarkdownFiles()
            .filter((f) => f.path.startsWith(config.folders.gallery))
            .map((f) => app.metadataCache.getFileCache(f) || {});
        return arrayUtil
            .uniqueArray(galleryMDFileCaches.flatMap((fc) => arrayUtil.safeArray((fc.frontmatter || {})[property])))
            .filter((v) => v).length;
    }
    /**
    ...
    getRenderedFolderPath(folder) {
        return folder.path
            .split('/')
            .map((part) => this.getRenderedFolderPathPart(part))
            .join('/');
    }
    /**
...
class FileProcesserUtil {
    static instance = SingletonFactory.getInstance('FileProcesserUtil', () => new FileProcesserUtil());
    static getInstance() {
        return this.instance;
    }
    /**
    ...
    processFileWith(getSpecTypeFileContent) {
        return async (file) => {
            ...
            const newData = await this.getFileContent(file, originalData, getSpecTypeFileContent);
    ...
    createFilesFromUnresolvedLinksForAllGalleryNoteFiles() {
        ...
        for (const linktext of arrayUtil.uniqueArray(unresolvedLinktexts)) {
            ...
            const propertyName = config.properties.find((pn) => galleryMDFileCaches.filter((fc) => arrayUtil.safeArray((fc.frontmatter || {})[pn]).includes(value)).length !== 0);
    ...
    batchMoveGalleryNoteFilesByYearUploaded() {
        ...
        const candidates = mdfiles.filter((f) => f.path.startsWith(config.folders.gallery));
        // First pass: create year folders
        for (const f of candidates) {
            if (f.path.split('/').length !== Constants.MARKDOWN_FILE_PATH_DEPTH)
                continue;
        ...
        for (const f of candidates) {
            ...
            files
                .filter((f2) => f2.path.startsWith(pathPrefix))
                .forEach((f2) => {
...
const arrayUtil = ArrayUtil.getInstance();
const pathUtil = PathUtil.getInstance();
const stringUtil = StringUtil.getInstance();
const fileProcesserUtil = FileProcesserUtil.getInstance();
const metadataCacheUtil = SingletonFactory.getInstance('MetadataCacheUtil', () => new MetadataCacheUtil());
/**
...
class ContentGenerator {
    static instance = SingletonFactory.getInstance('ContentGenerator', () => new ContentGenerator());
    static getInstance() {
        return this.instance;
    }
    /**
     ...
     */
    generateStandardContent(title, ctime, mtime, contentBody, preFMBlock = '') {
        return `---${preFMBlock}\nctime: ${ctime}\nmtime: ${mtime}\n---\n\n# ${title}\n\n${contentBody}`;
    }
    /**
     ...
     */
    generateContentWithBacklinksAndGallery(title, ctime, mtime, backlinksFilter, galleryFilter, additionalContent = '') {
     ...
     */
    async generateTagFileContent(title, ctime, mtime) {
        const baseTemplate = `!${config.refs.baseGalleryDynamic}\n\n`;
        return this.generateContentWithBacklinksAndGallery(title, ctime, mtime, i => !i.startsWith(config.folders.gallery) && i !== config.files.readme, i => i.startsWith(config.folders.gallery), baseTemplate);
    }
    /**
     ...
     */
    async generateYearFileContent(title, ctime, mtime) {
        ...
        const backlinks = app.metadataCache.getBacklinksForFile(f)?.data;
        const paths = backlinks ? [...backlinks.keys()] : [];
        const ngstr = pathUtil.getNonGalleryNotesStr(new Set(paths
     ...
     */
    async generateTagMetaFileContent(_title, ctime, mtime) {
     ...
     */
    generateGroupFileContent(title, ctime, mtime, seealso) {
    ...
    generateTagGroupIndex(title) {
        ...
        return uniqueValues
            ...
            .map((v) => `1. ${v} | ${galleryMDFileCaches.filter((fc) => arrayUtil.safeArray((fc.frontmatter || {})[property]).includes(v)).length}`)
            .join('\n');
    }
    /**
     ...
     */
    async generateTagGroupFileContent(title, ctime, mtime) {
     ...
     */
    async generateUploaderGroupFileContent(title, ctime, mtime) {
     ...
     */
    async generatePropertyFileContent(title, ctime, mtime) {
        const baseTemplate = `!${config.refs.basePropertyDynamic}\n`;
        return this.generateContentWithBacklinksAndGallery(title, ctime, mtime, i => !i.startsWith(config.folders.gallery) && i !== config.files.readme, i => false, baseTemplate);
    }
    /**
     ...
     */
    async generateReadmeFileContent(_title, ctime, mtime) {
        ...
        const newData = stringUtil
            ...
            .replace(Constants.REGEX_FOLDER_STAT_TABLE, '## folder-struct\n\n> DFC stands for the total number of descendant files\n\n' +
            tableStr +
            '\n');
        return newData;
    }
     ...
     */
    async generateGalleryNotesMetaFileContent(_title, ctime, mtime) {
        ...
        const gls = noteFiles
            ...
            .map((f) => f.path);
        const gstr = gls
            .map((p) => pathUtil.getGalleryItemRepresentationStr(p))
            .join('\n');
        const preFMBlock = `\nup:\n  - "${config.refs.docsCollection}"`;
        const newData = stringUtil
            .replaceFrontMatter(fileContent, ctime, mtime, preFMBlock)
            .replace(Constants.REGEX_FRONTMATTER_SECTION(config.keywords.noteList), `## ${config.keywords.noteList}\n\n${gstr}\n`);
        return newData;
    }
    /**
     ...
     */
    async generateGalleryMetaFileContent(_title, ctime, mtime, metaFilePath, galleryNoteFiles, preFMBlock = '') {
        const file = app.vault.getAbstractFileByPath(metaFilePath);
        const fileContent = await app.vault.read(file);
        const gstr = pathUtil.getGalleryItemsStr(new Set(galleryNoteFiles.map(f => f.path)));
        const newData = stringUtil
            .replaceFrontMatter(fileContent, ctime, mtime, preFMBlock)
            .replace(Constants.REGEX_FRONTMATTER_SECTION(config.keywords.galleryItems), `## ${config.keywords.galleryItems}\n\n${gstr}\n`);
        return newData;
    }
    /**
     ...
     */
    async generateGalleryItemsFileContent(_title, ctime, mtime) {
        ...
        return await this.generateGalleryMetaFileContent(_title, ctime, mtime, config.files.galleryItems, galleryNoteFiles, preFMBlock);
    }
    /**
     ...
     */
    async generateExhentaiGalleryFileContent(_title, ctime, mtime) {
        const galleryNoteFiles = metadataCacheUtil
            .getMarkdownFiles()
            .filter((f) => arrayUtil
            .safeArray(metadataCacheUtil.getFileCache(f)?.frontmatter?.up)
            .includes(config.refs.collectionGallery))
            .filter((f) => (metadataCacheUtil.getFileCache(f)?.frontmatter?.url || '').includes(config.keywords.exhentai));
        return await this.generateGalleryMetaFileContent(_title, ctime, mtime, config.files.exhentaiGallery, galleryNoteFiles);
    }
    /**
     ...
     */
    async generateNhentaiGalleryFileContent(_title, ctime, mtime) {
        const galleryNoteFiles = metadataCacheUtil
            .getMarkdownFiles()
            .filter((f) => arrayUtil
            .safeArray(metadataCacheUtil.getFileCache(f)?.frontmatter?.up)
            .includes(config.refs.collectionGallery))
            .filter((f) => (metadataCacheUtil.getFileCache(f)?.frontmatter?.url || '').includes(config.keywords.nhentai));
        return await this.generateGalleryMetaFileContent(_title, ctime, mtime, config.files.nhentaiGallery, galleryNoteFiles);
    }
    /**
     ...
     */
    generateFolderStatisticsTable(folders, files) {
...
const fileTemplateUtil = ContentGenerator.getInstance();
/**
...
class Main {
     ...
     */
    static main() {
        Main.asyncMain().catch(err => Main.logger.error('Unhandled exception during script execution:', err));
    }
    /**
     ...
     */
    static timedOperation(operationName, operation) {
        try {
     ...
     */
    static async timedAsyncOperation(operationName, operation) {
        try {
            const timerName = `${Constants.LOG_PREFIX_TIMER}${operationName}`;
            console.time(timerName);
            Main.logger.log(`${Constants.LOG_PREFIX_STARTED} ${operationName}`);
     ...
     */
    static async processSingleFile(path, fn) {
        const operationName = `Process file: ${path}`;
        await Main.timedAsyncOperation(operationName, () => fileProcesserUtil.getProcessFilePromise(path, fn));
    }
    /**
     ...
     */
    static async processDirectory(rootDirPath, fn) {
    ...
    static clearFrontmatter() {
        Main.timedOperation('Clear duplicate frontmatter properties', () => {
     ...
     */
    static async processSingleFiles() {
     ...
     */
    static async processDirectories() {
     ...
     */
    static async stageRefreshCache() {
     ...
     */
    static async stageBatchOperations() {
        await Main.timedAsyncOperation('Stage 2: Batch operations', async () => {
            await Promise.all([
                Promise.resolve(fileProcesserUtil.createFilesFromUnresolvedLinksForAllGalleryNoteFiles()),
                Promise.resolve(fileProcesserUtil.batchMoveGalleryNoteFilesByYearUploaded()),
     ...
     */
    static async stageSingleFileProcessing() {
        await Main.timedAsyncOperation('Stage 3: Single file processing', () => Main.processSingleFiles());
    }
    /**
     ...
     */
    static async stageDirectoryProcessing() {
        await Main.timedAsyncOperation('Stage 5: Directory processing', () => Main.processDirectories());
    }
    /**
     * Stage 6: Cleanup
